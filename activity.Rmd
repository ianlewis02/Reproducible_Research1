...
###   Q1a - Plot Histogram of the Total Number of Steps Taken per Day
```{r Q1a}
    ggplot(steps, aes(x = steps)) +
        ...
        labs(title = "Total Number of Steps Taken Each Day",
             ...
             y = "Occurances")
```
###   Q1b - Calculate and report the mean and median of the total number of steps taken per day
```{r Q1b}
...
```
###   Q2a - Make a time-plot of the 5-minute interval and average number of steps taken, averaged across all days
```{r Q2a}
    ggplot(interval, aes(x=interval, y=steps)) +
        ...
        labs(title = "Average Number of Steps taken (Averaged Across All Days)", 
             ...
             y = "Average Steps")
```
###   Q2b -  Which 5-minute interval, on average across all the days, contains the maximum number of steps?
```{r Q2b}
...
###   Q2b1 - Calculate and report the total number of missing values in the dataset 
```{r Q2b1}
    missing.steps <- sum(is.na(activity$steps))
    paste("The number of missing values in the dataset is",missing.steps)
```
###   Q2b2 - Devise a strategy for filling in all of the missing values in the dataset. 
```{r Q2b2}
...
```
###   Q2b3 - Create a new dataset (equal to the original) but with the missing data filled in.
```{r Q2b3}
    tofill.activity$steps[missing.activity] <- mean.interval[as.character(tofill.activity$interval[missing.activity])]
```
###   Q2b4 - Make a histogram of the total number of steps taken each day
```{r Q2b4}
    ...
    ggplot(filled_steps, aes(x = steps)) +
        ...
        y = "Occurances")
```
###   Q2b5 - Calculate and report the mean and median total number of steps taken per day   ###
```{r Q2b5}
    mean.steps <- round(mean(filled_steps$steps, na.rm=TRUE))
    median.steps <- round(median(filled_steps$steps, na.rm=TRUE))
    paste("The Mean number of steps is",mean.steps,"and the Median is",median.steps)
```
###   Q2b6 - Do these values differ from the estimates from the first part of the assignment?   ###
```{r Q2b6}
